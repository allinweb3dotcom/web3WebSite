...
const BnbRugCheckPage = () => {
  ...
  const handleCheck = async () => {
    ...
    try {
      const [securityResponse, infoResponse] = await Promise.all([
        axios.get(`https://preserver.mytokenpocket.vip/v1/token/token_security?address=${address}&ns=ethereum&chain_id=56`),
        axios.get(`https://preserver.mytokenpocket.vip/v1/token?blockchain_id=12&bl_symbol=${address}&address=${address}`)
      ]);
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      link.download = `bnb-rugcheck-${securityInfo?.token_symbol.toLowerCase() || 'result'}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`bnb-rugcheck-${securityInfo.token_symbol.toLowerCase()}.pdf`, {
        compress: true
      });
    } catch (err) {
  ...
  const renderSecurityStatus = (isSecure: boolean, text: string) => (
    <div className={`flex items-center gap-2 ${isSecure ? 'text-green-400' : 'text-red-400'}`}>
      {isSecure ? <CheckCircle className="w-5 h-5" /> : <AlertTriangle className="w-5 h-5" />}
      <span>{text}</span>
    </div>
  );
  ...
  const getSecurityScore = (securityInfo: TokenSecurity): number => {
    ...
    const checks = [
      ...
      { condition: securityInfo.owner_address === "0x0000000000000000000000000000000000000000", weight: 2 },
    ...
    const earnedWeight = checks.reduce((sum, check) => sum + (check.condition ? check.weight : 0), 0);
    ...
    <div className="min-h-screen py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-4">
          ...
          <h1 className="text-3xl font-bold gradient-text">BNB Token RugCheck</h1>
          <p className="mt-2 text-base text-gray-400">
            Check the safety and reliability of any BNB token
          </p>
        </div>
        ...
        <div className="bg-gray-800 bg-opacity-50 backdrop-blur-lg rounded-lg p-6 border border-gray-700 mb-8">
          <div className="flex gap-4">
            <input
              ...
              placeholder="Enter BNB token address..."
          ...
          <>
            <div ref={resultRef} className="space-y-6">
              <div className="bg-gray-800 bg-opacity-50 backdrop-blur-lg rounded-lg p-6 border border-gray-700">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="text-center">
                    ...
                    <div className={`text-3xl font-bold ${
                      getSecurityScore(securityInfo) >= 8 ? 'text-green-400' :
                      getSecurityScore(securityInfo) >= 6 ? 'text-yellow-400' :
                      'text-red-400'
                    }`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="bg-gray-700 bg-opacity-50 rounded-lg p-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={securityInfo.is_honeypot === "0" ? 'text-green-400' : 'text-red-400'}>
                ...
                <div className="mt-8">
                  <h3 className="text-xl font-bold text-white mb-4">Contract Security Report</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <div className="bg-gray-700 bg-opacity-50 rounded-lg p-4">
                        ...
                        <div className="space-y-3">
                          {renderSecurityStatus(securityInfo.is_open_source === "1", 
                            "Contract is open source")}
                          {renderSecurityStatus(securityInfo.is_proxy === "0", 
                            "No proxy contract")}
                          {renderSecurityStatus(securityInfo.owner_address === "0x0000000000000000000000000000000000000000", 
                            "Ownership renounced - No minting possible")}
                          {renderSecurityStatus(securityInfo.owner_change_balance === "0", 
                            "Cannot modify balances")}
                          {renderSecurityStatus(securityInfo.hidden_owner === "0", 
                            "No hidden owner")}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="bg-gray-700 bg-opacity-50 rounded-lg p-4">
                        ...
                        <div className="space-y-3">
                          {renderSecurityStatus(securityInfo.can_take_back_ownership === "0", 
                            "Cannot reclaim ownership")}
                          {renderSecurityStatus(securityInfo.selfdestruct === "0", 
                            "No self-destruct function")}
                          {renderSecurityStatus(securityInfo.external_call === "0", 
                            "No external calls")}
                          {renderSecurityStatus(securityInfo.is_honeypot === "0", 
                            "Not a honeypot")}
                          {renderSecurityStatus(securityInfo.transfer_pausable === "0", 
                    ...
                    <div className="space-y-4">
                      <div className="bg-gray-700 bg-opacity-50 rounded-lg p-4">
                        ...
                        <div className="space-y-3">
                          {renderSecurityStatus(securityInfo.anti_whale_modifiable === "1", 
                            "Anti-whale protection enabled")}
                          {renderSecurityStatus(securityInfo.personal_slippage_modifiable === "0", 
                            "No personal slippage modification")}
                          {renderSecurityStatus(securityInfo.trading_cooldown === "0", 
                            "No trading cooldown")}
                          <div className="flex justify-between items-center text-gray-300">
                            <span>Buy Tax:</span>
                            <span className={Number(securityInfo.buy_tax) > 0 ? 'text-yellow-400' : 'text-green-400'}>
                              {securityInfo.buy_tax}%
                            </span>
                          </div>
                          <div className="flex justify-between items-center text-gray-300">
                            <span>Sell Tax:</span>
                            <span className={Number(securityInfo.sell_tax) > 0 ? 'text-yellow-400' : 'text-green-400'}>
                    ...
                    <div className="space-y-4">
                      <div className="bg-gray-700 bg-opacity-50 rounded-lg p-4">
                        ...
                        <div className="space-y-3">
                          {renderSecurityStatus(securityInfo.cannot_buy === "0", 
                            "Buying is enabled")}
                          {renderSecurityStatus(securityInfo.cannot_sell_all === "0", 
                            "Selling is enabled")}
                          {renderSecurityStatus(securityInfo.is_in_dex === "1", 
                            "Listed on DEX")}
                          {renderSecurityStatus(securityInfo.is_blacklisted === "0", 
                            "No blacklist")}
                          {renderSecurityStatus(securityInfo.slippage_modifiable === "0", 
                            "Fixed slippage")}
                        </div>
                      </div>
                    </div>
                ...
                {securityInfo.is_in_dex === "1" && securityInfo.dex && (
                  <div className="mt-8">
                    <h3 className="text-xl font-bold text-white mb-4">DEX Liquidity</h3>
                        ...
                        <div key={index} className="bg-gray-700 bg-opacity-50 rounded-lg p-4">
                          <div className="flex justify-between items-center mb-2">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm text-gray-400">({dex.liquidity_type})</span>
                            </div>
                            <span className="text-white font-medium">
                              ${Number(dex.liquidity).toLocaleString(undefined, { maximumFractionDigits: 2 })}
                            </span>
                          </div>
                ...
                <div className="mt-8">
                  <h3 className="text-xl font-bold text-white mb-4">Top Holders</h3>
                  <div className="space-y-3">
                      ...
                      <div key={index} className="flex items-center gap-4">
                        ...
                        <div className="flex-1 bg-gray-700 rounded-lg h-2">
                          <div
                            ...
                            style={{ width: `${Number(holder.percent) * 100}%` }}
                          />
                        </div>
                        <div className="w-32 text-right">
                          <div className="text-white">{(Number(holder.percent) * 100).toFixed(2)}%</div>
...
export default BnbRugCheckPage;